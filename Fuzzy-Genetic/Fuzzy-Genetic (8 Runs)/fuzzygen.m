...
for i=1:5
powerknees(i,:) = sort(min(power) + (max(power)-min(power)).*rand(1,3));              
speedknees(i,:) = sort(min(speed) + (max(speed)-min(speed)).*rand(1,3));
durationknees(i,:) = sort(min(duration) + (max(duration)-min(duration)).*rand(1,3));
strengthknees(i,:) = sort(min(strength) + (max(strength)-min(strength)).*rand(1,3));
widthknees(i,:) = sort(min(width) + (max(width)-min(width)).*rand(1,3));
depthknees(i,:) = sort(min(depth) + (max(depth)-min(depth)).*rand(1,3));
end
...
pkl = de2bi(round(a*powerknees(:,1)),length); % Power Knees Low
pkm = de2bi(round(a*powerknees(:,2)),length); % Power Knees Medium
pkh = de2bi(round(a*powerknees(:,3)),length); % Power Knees High

sks = de2bi(round(b*speedknees(:,1)),length);
ska = de2bi(round(b*speedknees(:,2)),length);
skf = de2bi(round(b*speedknees(:,3)),length);

dks = de2bi(round(c*durationknees(:,1)),length); % Duration Knees Small
dka = de2bi(round(c*durationknees(:,2)),length); % Duration Knees Average
dkb = de2bi(round(c*durationknees(:,3)),length); % Duration Knees Big

skl = de2bi(round(d*strengthknees(:,1)),length);
skm = de2bi(round(d*strengthknees(:,2)),length);
skh = de2bi(round(d*strengthknees(:,3)),length);

wkl = de2bi(round(e*widthknees(:,1)),length);
wkm = de2bi(round(e*widthknees(:,2)),length);
wkh = de2bi(round(e*widthknees(:,3)),length);

dkl = de2bi(round(f*depthknees(:,1)),length);
dkm = de2bi(round(f*depthknees(:,2)),length);
dkh = de2bi(round(f*depthknees(:,3)),length);
...
[fit,knee,st,wi,de] = fitness(newpop,sizes,inputs,outputs,k,count); % 
...
plot1 = plot(axes1,xpol,ypol,'Marker','.','LineWidth',3,'Color',[0 0 0],...
                            'MarkerEdgeColor','w',...
                        'MarkerFaceColor','k');
plot2 = plot(axes1,xpol,ypom,'Marker','.','LineWidth',3,'Color',[0 0 0],...
                            'MarkerEdgeColor','w',...
                        'MarkerFaceColor','k');
...
while min(fit) > mse && trial < 10000 % until mse is met of number of trials exceed the specified number do the following
     newpop = crossover(newpop,fit,pc,sizes,k,power,speed,duration,strength,width,depth); % crossover operation
     ...
     [fit,knee,st,wi,de] = fitness(newpop,sizes,inputs,outputs,k,count); % function to find out if the mse is reached
