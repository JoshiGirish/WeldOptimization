function newpop = crossover4(pop,fit,pc,sizes,k,power,speed,duration,strength,width,depth)
...
%          newpop(indc(2),:) =[pop(index(2),1:cpoint) pop(index(1),cpoint+1:end)];
        ...
        newpop(indc(1),:) = test4(child1,sizes,k,power,speed,duration,strength,width,depth);
        newpop(indc(2),:) = test4(child2,sizes,k,power,speed,duration,strength,width,depth);
        end
